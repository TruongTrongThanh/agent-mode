...
- **Keyboard shortcuts** — `Ctrl+Shift+M` to cycle, `Ctrl+Shift+S` to search agents
- **Visual indicator** — Widget banner above editor shows active agent
- **Model & tool restriction** — Per-agent model and tool sets
- **Agent search** — `/agent-search <query>` or `Ctrl+Shift+S` to find agents by name, description, or body content
...
- `Ctrl+Shift+S` — Search agents (opens query prompt, then shows ranked results)
