...
- **Keyboard shortcuts** — `Ctrl+Shift+M` to cycle, `F2` to search agents
- **Visual indicator** — Widget banner above editor shows active agent
- **Model & tool restriction** — Per-agent model and tool sets
- **Agent search** — `/agent-search <query>` or `F2` to find agents by name, description, or body content
...
- `F2` — Search agents (opens query prompt, then shows ranked results)
