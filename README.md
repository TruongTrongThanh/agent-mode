...
- **Keyboard shortcuts** — `Ctrl+Shift+M` to cycle, `Ctrl+Alt+F` to search agents
- **Visual indicator** — Widget banner above editor shows active agent
- **Model & tool restriction** — Per-agent model and tool sets
- **Agent search** — `/agent-search <query>` or `Ctrl+Alt+F` to find agents by name, description, or body content
...
- `Ctrl+Alt+F` — Search agents (opens query prompt, then shows ranked results)
